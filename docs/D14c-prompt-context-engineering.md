...
### Thinking Mode (Modalità di Ragionamento) vs Instruct Mode

Sempre nell'ambito della strutturazione del System Prompt, è fondamentale padroneggiare le due macro-modalità operative per istruire i modelli (specialmente se eseguiti localmente):
- **Thinking Mode (CoT / Chain of Thought)**: Diciamo esplicitamente al modello (tramite prompt e parametri) di fermarsi a "pensare a voce alta" (spesso tra tag `<think>...</think>`) prima di generare l'output. Questo riduce enormemente le allucinazioni in compiti matematici o logici complessi. *Parametri ottimali*: Top-P e Temperatura leggermente più alti, per permettere al modello di esplorare varie diramazioni di ragionamento. Attenzione al rischio di *Overthinking* (quando il modello entra in loop o ragiona per troppi token inutilmente).
- **Instruct Mode (No Thinking)**: Il modello viene spinto a fornire la risposta diretta e risolutiva immediatamente, senza passaggi intermedi. È preferibile per query semplici, formattazione di testi o estrazione dati (dove la latenza è il vincolo principale). *Parametri ottimali*: Temperatura bassissima (es. 0.1 o 0.0) e Top-K basso per massimizzare il determinismo e minimizzare i tempi di latenza (*Time-To-First-Token*).
